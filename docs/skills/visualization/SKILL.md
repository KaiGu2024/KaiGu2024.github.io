...
4. **Direct annotation, no legends.** Label each line/group at its endpoint or most legible point. Suppress redundant guides with `guides(fill = "none")` when a variable is mapped to both `x` and `fill`/`color`.
    ...
    **Annotation styling — size, font, parentheses.** Use `geom_text(size = 7–8)` ≈ **20–22 pt rendered** for direct line/point labels and inline annotations: slightly smaller than axis tick labels (24 pt) so commentary doesn't compete with the reference axis. Direct *endpoint* labels that replace a legend can match the axis text at `size = 8` (~22 pt) since they ARE the identification system. Use the same font family as the rest of the figure (Helvetica via `theme_pub()`); don't switch to a serif or display face inside the data field — figures stay in a clean sans regardless of the body font of the paper they sit in. Parentheses **only for qualifiers or units**: `Mortgage amount (USD, log)`, `GDP (Δ %)`, `ChatGPT (US users)`. Never around the primary noun itself — write `ChatGPT`, not `(ChatGPT)`. Brackets and parens are visual noise unless they're carrying a sub-meaning.

    **Label what needs identifying, not every series.** "No legends" means *replace the legend with direct labels* — it does not mean *label every line*. In layer-and-highlight, the grey80 background series are deliberately unlabeled; they're context, not identity. In a 2-series treatment/control comparison, label both. In horizontal bar charts where y-axis ticks already name each bar (countries, brands, models), the ticks ARE the labels — don't add a second layer. If a panel has so many series that labeling every one creates clutter, that's a sign to facet or to switch to layer-and-highlight; don't squeeze in more labels.
