...
7. [Output](#7-output) — one standalone figure per file, ggsave at 600 DPI, one script per figure
...
10. **Axis titles only — no in-figure text.** The only text inside the image is axis titles (and facet strips if a single-file facet is unavoidable). **Do not** set `plot.title`, `plot.subtitle`, `plot.caption`, or `plot.tag`; those belong in LaTeX (`\caption{}`, section prose, slide chrome) where they can be re-edited without re-rendering the figure. Notes, sample descriptions, model details, and any panel letter all live in TeX, not in the PNG/PDF.
    ...
    **Axis text — concrete requirements.** Axis titles and tick labels are the figure's only words, so make them clear, short, and precise:
    - **Sentence case.** Capitalize the first word and proper nouns only — `Mortgage amount (USD, log)`, not `Mortgage Amount` and not `MORTGAGE AMOUNT` (US Data Visualization Standards; RSS *Best Practices for Data Visualisation*).
    - **Unit in parentheses, at the end.** `Quantity (unit)` — `Referral share (%)`, `Response time (ms)`, `Daily visits (thousands)`. Use standard/SI nomenclature, one space inside unit expressions (Nature figure guidelines).
    - **Spell the quantity out — no cryptic abbreviations.** `Response time`, not `RT`; `Referral share`, not `ref_sh`.
    - **Length 3–6 words.** The 3-word-plus-unit floor stays; the ceiling is ~6 words — anything longer is a caption and belongs in TeX. Drop dead prefixes (`Number of`, `Value of`) when the noun or unit already carries the meaning: `Daily visits`, not `Number of daily visits`.
    - **Tick labels short and fixed-width.** Format with `scales::label_*` — `label_percent()`, `label_dollar()`, `label_number(scale_cut = cut_short_scale())` for k/M/bn — never raw digit strings or scientific-notation tofu (`1e+05`). Dates stay ISO `%Y-%m` (rule 7). Aim ≤ ~6 characters per tick label. Recipe in `references/recipes.md` → "Axis text — number formatting".
    - **`y = NULL` only when the ticks already name each item** — sorted horizontal bars, where the y tick labels ARE the axis title. Otherwise the y-axis gets a real 3–6-word title.

    **One standalone figure per file.** Faceting and patchwork combining are out of the default toolbox: each chart exports as its own file and goes into LaTeX as its own `figure` float — no `subfigure`, no panel letters, no combined floats. Two related charts sit as two consecutive floats, not one panel grid. If a faceted layout is truly the right encoding (small multiples over a single ordered variable), export the faceted plot as one file — that is still one standalone figure, not a subfigure.
...
The theme function, brand palette, sequential ramp, and global ggplot defaults all live in `scripts/theme_pub.R`. Source it from a shared `_setup.R` (see "One script per figure" below) — don't copy-paste from this file, the script is the source of truth.
...
library(ggrepel)
library(scales)
# Note: no patchwork — one standalone figure per file, its own LaTeX float.
...
**One runnable script per figure.** Each figure is produced by its own self-contained script (e.g. `figures/fig_<slug>.R`) that writes exactly **one** figure file. Never put several figures in one serial script — that forces them to render one after another.

**Shared setup, not a shared run.** Factor the common preamble — the `library()` calls, `source(".../theme_pub.R")`, the `showtext` font registration above, and any shared data prep — into one `_setup.R` that each figure script `source()`s at the top. Cache expensive data prep to disk (e.g. `saveRDS()` / `readRDS()`) so each job reads the prepared data instead of recomputing it.
...
# ... build p ...
ggsave("fig_referral_share.pdf", p, width = 4.5, height = 3.3,
       units = "in", device = cairo_pdf)
```

Because each script is an independent unit, the figures render **simultaneously** under whatever parallel launcher you prefer (the launcher is yours to choose). This mirrors the output rule: one figure = one file = one script = one job.
...
**One standalone figure per file.** No patchwork, no `plot_annotation(tag_levels = "A")`, no in-R combining. Each chart goes to its own PDF/PNG and enters LaTeX as its own `figure` float — no `subfigure`.
...
# figure (~3.3 in), author width = 4–4.5 in; for a column figure, 6–7 in.
ggsave("fig_referral_share.pdf", plot = p, width = 4.5, height = 3.3,
       units = "in", device = cairo_pdf)
ggsave("fig_referral_share.png", plot = p, width = 4.5, height = 3.3,
       units = "in", dpi = 600)
```

**LaTeX placement** — one figure, one float:

```latex
\begin{figure}[t]
  \centering
  \includegraphics[width=0.6\linewidth]{fig_referral_share.pdf}
  \caption{Headline finding. Source / sample / model lives here, not in the PNG.}
  \label{fig:referral-share}
\end{figure}
```

Two related charts are **two consecutive floats**, each with its own `\caption{}` — not one combined `subfigure` grid. The caption, the source note, and any cross-reference label are all TeX; nothing (title, note, panel letter) is ever baked into the figure file.
